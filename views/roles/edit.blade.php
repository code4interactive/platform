...
@section('page-heading')
    <div class="row wrapper border-bottom white-bg page-heading">
        <div class="col-lg-10">
            <h2>Edycja roli <strong>"{{$role->name}}"</strong></h2>
            <ol class="breadcrumb">
                <li>
                    <a href="/">Home</a>
                </li>
                                    ...
                                    <table class="table">
                                        ...
                                        <tbody id="collapse{{$lp}}" class="collapse" >
                                            @foreach($val['permissions'] as $p => $v)
                                                <?php
                                                    $itemPerm = "notset";
                                                    if (array_key_exists($p, $role->permissions)) {
                                                        $itemPerm = $role->permissions[$p];
                                                    }
                                                ?>
                                            <tr>
                                                <td>{{$v}}</td>
                                                <td><input type="radio" name="permission[{{$per}}][{{$p}}]" value="true" {{$itemPerm===true?"checked":""}} ></td>
                                                <td><input type="radio" name="permission[{{$per}}][{{$p}}]" value="false" {{$itemPerm===false?"checked":""}} ></td>
                                                <td><input type="radio" name="permission[{{$per}}][{{$p}}]" value="notset" {{$itemPerm==="notset"?"checked":""}}></td>
                                            </tr>
                                            @endforeach
                                        </tbody>
                                        <?php $lp++; ?>
